...
def get_num_of_lesson(book: List[Cell], cell: Cell) -> Cell:
    ...
    num_of_lesson = find_cell(
        book=book, sheet=cell.sheet, row=cell.row - 1, col=cell.col - 4
    )
...
def get_type_of_week(book: List[Cell], cell: Cell) -> Cell:
    ...
    type_of_week = find_cell(
        book=book, sheet=cell.sheet, row=cell.row - 1, col=cell.col - 3
    )
    if (
        type_of_week.value == ""
        and find_cell(
            book=book, sheet=cell.sheet, row=cell.row - 1, col=cell.col - 2
        ).top_line_style
        != "thin"
    ):
        iteration = 0
...
def get_type_of_lesson(book: List[Cell], cell: Cell) -> Cell:
    ...
    type_of_lesson = find_cell(
        book=book, sheet=cell.sheet, row=cell.row, col=cell.col - 2
    )
...
def get_name_of_lesson(book: List[Cell], cell: Cell) -> Cell:
    ...
    if (
        lesson_name.top_line_style != "thin"
        and lesson_name.top_line_style != "dashDot"
    ):
        iteration = 0
        while True:
            if (
...
def get_day_of_week(book: List[Cell], cell: Cell) -> Cell:
    ...
    while True:
        ...
        day_of_week = find_cell(
            book=book, sheet=cell.sheet, row=cell.row - iteration, col=cell.col
        )
...
def get_dates(book: List[Cell], cell: Cell) -> Cell:
    ...
    if "." not in dates.value:
        ...
        while True:
            ...
            if (
                dates_cell.top_line_style == "thin"
                or dates_cell.top_line_style == "dashDot"
            ):
                if "." in str(dates_cell.value).strip():
                    if ", т" in str(dates_cell.value).strip():
                        dates = dates_cell
                        dates.value = (
                            f"т" f'{str(dates.value).strip().split(", т")[1]}'
                        )
                    if ", с" in str(dates_cell.value).strip():
                        dates = dates_cell
                        dates.value = (
                            f"с" f'{str(dates.value).strip().split(", с")[1]}'
                        )
                break
...
def get_dates_list(
    book: List[Cell], dates: Cell, day_of_week: Cell, type_of_week: Cell
) -> Cell:
    if "только" in dates.value:
        dates.value = only(dates_value=dates.value, book=book)

    if "с" in dates.value:
        dates.value = since(
            dates_value=dates.value,
    ...
    if isinstance(dates.value, str):
        if (
            "кроме" in dates.value
            and "с " not in dates.value
            and "по " not in dates.value
        ):
            dates.value = []
    return dates
