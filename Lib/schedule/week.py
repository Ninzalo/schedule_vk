...
def schedule_week(path: str, group, schedule, font_path: str) -> None:
    ...
    for subgroup in range(1, subgroups + 1):
        ...
        for quality in qualities:
            if not os.path.exists(f"{path}/{group}/" f"s{subgroup}/{quality}"):
            ...
            for delta in range(0, max_delta + 1, 7):
                ...
                days = []
                days = [
                    (today_date - datetime.timedelta(days=delta)).strftime(
                        "%Y-%m-%d"
                    )
                ...
                dict_file = file_input(
                    file_path=schedule, days=dates, subgroup=subgroup
                )
                ...
                for mode in modes:
                    ...
                    if not os.path.exists(
                        f"{path}/{group}/" f"s{subgroup}/{quality}/{mode}"
                    ):
                        os.mkdir(
                            f"{path}/{group}/s{subgroup}/" f"{quality}/{mode}"
                        )
                    output_path = (
...
def gen_image(output_path, font_path, mode, dict_file, quality):
    ...
    for card in dict_file:
        ...
        date_block = [
            [f'{lessons_list[0]["day_of_week"].capitalize()}'],
            [f"{date}"],
            [f'{lessons_list[0]["type_of_week"]}'],
        ]
        ...
        for input_json in lessons_list:
            lesson_in_day = []
            input_data_list = [
                input_json["lesson_name"],
                input_json["type_of_lesson"],
                input_json["name"],
                input_json["room"],
            ]
    ...
    if (
        final_image_width == 0
        or final_image_height == 0
        or len(days_in_week_list) == 1
    ):
        return
    ...
    if mode == "night":
        fill_color = "white"
        image = Image.new(
            "RGB", (final_image_width, final_image_height), (0, 0, 0)
        )
    else:
        fill_color = "black"
        image = Image.new(
            "RGB", (final_image_width, final_image_height), (255, 255, 255)
        )
    # draw text
    rows = 0
