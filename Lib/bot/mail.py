...
def _send_to_target(password: str, user_id: int, text: str, 
    attachments: list|None) -> Returns:
    target_ids = db.get_all_users_with_pass(password=password)
    result = Returns()
    for target_id in target_ids:
        try:
            result.add_return(user_id=target_id,
                text=text,
        ...
        if event.message.split(' ')[0] == password:
            ...
            if password_privacy == 1:
                ...
                result.returns += _send_to_target(
                    ...
                    attachments=attachments).returns
    if result.returns == []:
        result.add_return(user_id=user_id, 
            text='Вы не добавили данный пароль')
    return result
    ...
    for user_id, form, fac, group, subgroup in users_for_daily_mail:
        ...
        for date in data:
            ...
            if file_year > date_year:
                ...
                new_data.append(date)
            elif (file_year == date_year and 
                file_month == date_month and 
                file_day >= date_day):
                new_data.append(date)
        if len(new_data) != 0:
            delta = 1
