from Lib.bot.BotDataBase import BotDB, notifications, teacher_search
...
def _wrapper(key: int|None) -> Error_handler:
    errors = {
...
class Notifications(BotDB_Func):
    def get(self, user_id: int) -> bool:
        """ Получаем notifications пользователя """
        with notifications() as db:
            result = db.get(user_id=user_id)
            return result 

    def change(self, user_id: int) -> None:
        """ Меняем notifications пользователя """
        with notifications() as db:
            db.change(user_id=user_id)

class Teacher_search(BotDB_Func):
    def add_user(self, user_id: int) -> int:
        with teacher_search() as db:
            if not db.user_exists(user_id=user_id):
